...
struct LobbyBindingsCallbacks {
  ...
  std::function<void(ILobbyView::NavigationTarget)> on_navigation_selected;
  ...
  std::function<void(const RoomCreatePayload&)> on_my_rooms_create;
  std::function<void()> on_my_rooms_refresh;

  std::function<void()> on_entry_back;
  std::function<void(IRoomEntryView::EntrySelection)> on_entry_join;
