...
class LobbyDataController {
 public:
  LobbyDataController(RoomApiClient& room_api, IceConfigClient& ice_client,
                      RoomCatalog& room_catalog, ILobbyView& lobby_view,
                      IMyRoomsView& my_rooms_view,
 ...
 private:
  ...
  ILobbyView& lobby_view_;
  IMyRoomsView& my_rooms_view_;
  std::shared_ptr<std::atomic_bool> alive_flag_;
  AppStatusContext& status_context_;
  std::function<void(const std::string&)> set_lobby_status_text_;
