...
void RoomApiClient::fetchRoomsAsync(const RoomListFilter& filter,
  ...
  std::thread([base_url = base_url_, filter, callback = std::move(callback)]() mutable {
...
void RoomApiClient::createRoomAsync(const std::string& host_user_id,
  ...
  std::thread([base_url = base_url_, host_user_id, max_participants,
    ...
    request.body = roomapi::BuildCreateRoomBody(host_user_id, max_participants);
    request.timeout_ms = 3500;
...
void RoomApiClient::joinRoomAsync(const std::string& room_code,
  ...
  std::thread([base_url = base_url_, room_code, user_id,
               callback = std::move(callback)]() mutable {
    juce::String error_text;
    const bool ok = postUserAction(
        base_url, "/api/rooms/" + http::EscapePathSegment(room_code) + "/join", user_id,
        error_text);
...
void RoomApiClient::leaveRoomAsync(const std::string& room_code,
  ...
  std::thread([base_url = base_url_, room_code, user_id,
    ...
    juce::String error_text;
    const bool ok = postUserAction(
        base_url, "/api/rooms/" + http::EscapePathSegment(room_code) + "/leave", user_id,
        error_text);
...
void RoomApiClient::updateRoomAsync(const std::string& room_code,
                                    ...
                                    SimpleCallback callback) const {
  std::thread([base_url = base_url_, room_code, host_user_id, payload,
               callback = std::move(callback)]() mutable {
    juce::String error_text;
    const juce::String body = roomapi::BuildUpdateRoomBody(host_user_id, payload);
    const bool ok =
        payloadAction(base_url, "PUT", "/api/rooms/" + http::EscapePathSegment(room_code), body,
                      error_text);
...
void RoomApiClient::deleteRoomAsync(const std::string& room_code,
                                    const std::string& host_user_id,
                                    SimpleCallback callback) const {
  std::thread([base_url = base_url_, room_code, host_user_id,
               callback = std::move(callback)]() mutable {
    juce::String error_text;
    const juce::String body = roomapi::BuildDeleteRoomBody(host_user_id);
    const bool ok =
        payloadAction(base_url, "DELETE",
...
bool RoomApiClient::postUserAction(const std::string& base_url,
                                   ...
                                   const std::string& user_id,
                                   juce::String& error_text) {
  return payloadAction(base_url, "POST", endpoint, roomapi::BuildUserActionBody(user_id),
                       error_text);
}
...
bool RoomApiClient::payloadAction(const std::string& base_url,
  ...
  if (!response.isHttpSuccess()) {
    error_text =
        juce::String(tempolink::juceapp::text::kRoomActionApiHttpErrorPrefix) +
        juce::String(response.status_code);
    return false;
  }
  return true;
