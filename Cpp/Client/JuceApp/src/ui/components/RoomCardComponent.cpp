...
RoomCardComponent::RoomCardComponent() {
  ...
  description_label_.setColour(juce::Label::textColourId,
                               tempolink::juceapp::style::TextSecondary());
  description_label_.setMinimumHorizontalScale(0.9F);
  ...
  preview_button_.onClick = [this] {
    if (on_preview_) {
      on_preview_(room_.room_code);
    }
  };
  enter_button_.onClick = [this] {
    if (on_enter_) {
      on_enter_(room_.room_code);
    }
  };
  edit_button_.onClick = [this] {
    if (on_edit_) {
      on_edit_(room_.room_code);
    }
  };
  delete_button_.onClick = [this] {
    if (on_delete_) {
      on_delete_(room_.room_code);
    }
  };
  share_button_.onClick = [this] {
    if (on_share_) {
      on_share_(room_.room_code);
    }
  };
...
void RoomCardComponent::setRoom(const RoomSummary& room) {
  ...
  description_label_.setText(room_.description.isNotEmpty() ? room_.description : "No description",
                             juce::dontSendNotification);
  meta_label_.setText(room_.genre + "  |  " + juce::String(room_.members) + "/" +
                          juce::String(room_.capacity) + "  |  " +
                          (room_.has_password ? "Password" : "Open"),
                      juce::dontSendNotification);
  host_label_.setText("Host: " + room_.host_label, juce::dontSendNotification);
  latency_label_.setText(
      "Tags: " + (tags_text.isNotEmpty() ? tags_text : juce::String("-")) +
          "  |  " + room_.latency_hint,
                         juce::dontSendNotification);
  repaint();
}
...
void RoomCardComponent::setPreviewHandler(std::function<void(std::string)> on_preview) {
  on_preview_ = std::move(on_preview);
}

void RoomCardComponent::setEnterHandler(std::function<void(std::string)> on_enter) {
  on_enter_ = std::move(on_enter);
}

void RoomCardComponent::setEditHandler(std::function<void(std::string)> on_edit) {
  on_edit_ = std::move(on_edit);
}

void RoomCardComponent::setDeleteHandler(std::function<void(std::string)> on_delete) {
  on_delete_ = std::move(on_delete);
}

void RoomCardComponent::setShareHandler(std::function<void(std::string)> on_share) {
  on_share_ = std::move(on_share);
}
