...
```text
TempoLink/
  Cpp/                  # 실시간 오디오 데이터 플레인 (Client/Server)
...
1. `Cpp/Server`: UDP 오디오 릴레이 서버
2. `Cpp/Client/JuceApp`: JUCE 기반 클라이언트
3. `Cpp/Client/RealtimeCore`: C++ 실시간 세션 코어 + 콘솔 클라이언트
4. `Cpp/Shared`: Client/Server 공통 C++ 코드
5. `SpringBoot/ControlPlaneService`: 컨트롤 플레인 API(현재는 방 생성/입장/퇴장 중심)
...
6. `GET /api/system/relay-endpoint`
